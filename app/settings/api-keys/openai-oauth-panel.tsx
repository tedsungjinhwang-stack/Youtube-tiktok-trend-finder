      ...
      <div className="flex items-center justify-between gap-2">
        <div className="text-[13px]">
          ...
          {status ? (
            status.oauth.connected ? (
              <div className="mt-0.5 text-[13px] text-emerald-400">
        ...
        {status?.oauth.connected ? (
          <button
            type="button"
            onClick={disconnect}
            className="rounded border border-red-500/40 bg-red-500/10 px-2 py-1 text-[13px] text-red-300 hover:bg-red-500/20"
          >
            연결 해제
          </button>
        ) : (
        ...
        <div className="space-y-2 rounded border bg-background/40 p-2.5">
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded bg-foreground px-2.5 py-1 text-[13px] font-semibold text-background"
            >
          ...
          {pollMsg && (
            <div className="text-[13px] text-muted-foreground">{pollMsg}</div>
          )}
        </div>
      )}

      {err && (
        <div className="rounded border border-red-500/40 bg-red-500/10 px-2 py-1.5 text-[13px] text-red-300">
          {err}
        </div>
      )}
    </div>
