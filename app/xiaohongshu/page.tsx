...
export default async function XiaohongshuPage({
  ...
  const folders = await getFoldersWithChannelCount(['XIAOHONGSHU']);
  const c = cookies();
  const defaults = {
    ...
    <>
      ...
      <div className="px-4 py-4">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-bold tracking-tight">샤오홍수</h1>
            <p className="mt-0.5 text-xs text-muted-foreground">
              등록된 샤오홍수 채널 영상 · 폴더·기간·임계치 필터링
            </p>
          </div>
          <ScrapeButton platforms={['XIAOHONGSHU']} label="샤오홍수 채널 조회" />
        </div>

        <div className="mb-4">
          <PageFilters platforms={['XIAOHONGSHU']} showPlatformToggle={false} defaults={defaults} />
        </div>
  ...
  return (
    <div className="rounded-xl border border-dashed py-16 text-center text-sm text-muted-foreground">
      ...
      <p className="mt-1 text-xs">
        샤오홍수 채널을 추가하고 스크래핑하면 여기에 표시됩니다 —
